...
import {IBookResponse, IGenreResponse, PaginationInput} from "../../../models/auth.model";
        ...
        this.bookService.fetchBooks(paginationInput).subscribe({
            next: (res) => {
                ...
                res.forEach((book: any) => {
                    ...
                    this.genresMap.set(book.id, book.genres.map((genre: IGenreResponse) => genre.name).join(', '));
                })
            },
            error: (err) => {
                if (err.status === 404) {
